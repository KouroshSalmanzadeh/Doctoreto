...
export default function Page({ params }: Props): ReactElement {
    ...
    const { name, avatar, expertise, services, star, reviewCount, address, about, phone, services_types, reviews } = doctor;

    return (
        <div className={styles.page}>
            <div className={styles["right-section"]}>
                <div className={styles["doctor-info"]}>
                    {avatar && <Image src={avatar} alt={name} width={72} height={72} className={styles["doctor-avatar"]} />}
                ...
                <div className={styles["about-services-container"]}>
                    <div className={styles["about-section"]}>
                        ...
                        <div className={styles["about-text"]}>{about.split("،").map((item, i) => <p key={i} className={styles["about-item"]}><span className={styles["bullet-animate"]}></span>{item}</p>)}</div>
                    </div>
                ...
                <div className={styles["reviews-section"]}>
                    ...
                    <div className={styles["reviews-info"]}>
                        ...
                        <ProgressBarComponent label='رفتار حرفه ای پزشک (برخورد صبورانه و محترمانه)' rate={reviews.criteria.professional_behavior} />
                        <ProgressBarComponent label='اختصاص وقت و توضیحات کافی برای مراجعه کننده' rate={reviews.criteria.time_allocation} />
                        <ProgressBarComponent label='مهارت پزشک در تشخیص و درمان' rate={reviews.criteria.diagnostic_skill} />
                        <ProgressBarComponent label='فرایند پذیرش و رفتار منشی و پرسنل' rate={reviews.criteria.reception_process} />
                        <ProgressBarComponent label='شرایط محیط مطب و اتاق انتظار' rate={reviews.criteria.clinic_conditions} />
                    </div>
                        ...
                        {services_types.map((serviceType, index) => (
                            <div key={index} className={styles["service-type-item"]}>
                                <h3 className={styles["service-type-title"]}>
                                    {serviceType.type === 'online' ? <MingcuteChat1Line /> : serviceType.type === 'inPerson' ? <MingcuteHospitalLine /> : <MingcutePhoneCallLine />}
                                    {serviceType.type === 'online' ? 'مشاوره متنی' : serviceType.type === 'inPerson' ? "ویزیت حضوری" : 'ویزیت آنلاین'}
                                </h3>
                                <p className={styles["next-slot"]}>
                                    اولین نوبت آزاد:
                                    <span>
                                        {serviceType.next_slot}
                                    </span>
                                </p>
                                ...
                                <div className={styles["service-features"]}>
                                    <span className={styles["badge-item"]}>{serviceType.details_type.description}</span>
                                    {serviceType.details_type.features?.map((feature, index) => (
                                        <span key={index} className={styles["badge-item"]}>{feature}</span>
                                    ))}
                                </div>
                                <ButtonComponent>{serviceType.type === 'online' ? 'درخواست مشاوره' : serviceType.type === 'inPerson' ? "نوبت دهی" : 'مشاوره آنلاین'}</ButtonComponent>
                            </div>
                        ))}
                    </div>
                </div>
